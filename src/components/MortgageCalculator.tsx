          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">Прирост на взнос</p>
                  <p className="text-lg font-bold text-emerald-600">
                    +{result.investmentReturnPercent.toFixed(1)}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">ROI (вложения vs итог)</p>
                  <p className={`text-lg font-bold ${roiPositive ? "text-emerald-600" : "text-rose-600"}`}>
                    {result.roi > 0 ? "+" : ""}
                    {result.roi.toFixed(1)}%
                  </p>
                  <p className="text-xs text-muted-foreground">
                    (стоимость − {formatMoney(params.downPayment + result.totalPaid)}) / вложения
                  </p>
                </div>
              </div>
            </CardContent>
